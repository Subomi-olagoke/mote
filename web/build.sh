...
# mote_lib. Emscripten's serial fallback in parallel.c applies (no threads yet),
# and the NEON paths compile out, so this is the scalar engine plus whatever
# -msimd128 autovectorizes. Produces web/mote.js + web/mote.wasm.
set -e
cd "$(dirname "$0")/.."

emcc -O3 -std=c11 -msimd128 \
